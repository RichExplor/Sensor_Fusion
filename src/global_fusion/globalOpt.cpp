...
GlobalOptimization::GlobalOptimization()
{
	initGPS = false;
    newGPS = false;
	WGPS_T_WVIO = Eigen::Matrix4d::Identity();  // VIO坐标系转换为GPS坐标系。默认为单位阵，可以根据实际情况改动
    threadOpt = std::thread(&GlobalOptimization::optimize, this);  //  单独线程启动，优化位姿
}
...
{
	double xyz[3];
	GPS2XYZ(latitude, longitude, altitude, xyz);  // 将经纬度/高度转换为xyz坐标
	vector<double> tmp{xyz[0], xyz[1], xyz[2], posAccuracy};
    //printf("new gps: t: %f x: %f y: %f z:%f \n", t, tmp[0], tmp[1], tmp[2]);
	GPSPositionMap[t] = tmp;   // GPS全局坐标
    newGPS = true;

}

// add by gf
void GlobalOptimization::inputLOAM(double t, double x, double y, double z, double posAccuracy)
{
...
void GlobalOptimization::optimize()
{
    while(true)
    {
        if(newGPS)
        {
            ...
            //add param
            mPoseMap.lock();
            int length = localPoseMap.size();   // localPoseMap一直在增长,由VIO位姿数量决定
            // cout<<"localMap length = "<<length<<endl;
            // w^t_i   w^q_i
            double t_array[length][3];
            double q_array[length][4];
            map<double, vector<double>>::iterator iter;
            iter = globalPoseMap.begin();   // 已经变换到GPS坐标系下的VIO位姿
            ...
            for (iterVIO = localPoseMap.begin(); iterVIO != localPoseMap.end(); iterVIO++, i++)
            {
                ...
                if(iterVIONext != localPoseMap.end())
                {
                    ...
                    Eigen::Matrix4d iTj = wTi.inverse() * wTj;  // T_ij,j时刻到i时刻的变换
                    ...
                    // 2. 构建相对旋转误差
                    // ceres::CostFunction* vio_function = RError::Create( iQj.w(), iQj.x(), iQj.y(), iQj.z(),
                    //                                                              0.01);
                    // problem.AddResidualBlock(vio_function, NULL, q_array[i], q_array[i+1]);
                ...
                iterGPS = GPSPositionMap.find(t);  // 查找当前时刻是否存在GPS消息
                if (iterGPS != GPSPositionMap.end())
                {
                    ceres::CostFunction* gps_function = TError::Create(iterGPS->second[0], iterGPS->second[1], 
                                                                       iterGPS->second[2], iterGPS->second[3]);  // xyz位置和精度
