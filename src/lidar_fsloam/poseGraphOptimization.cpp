...
ros::Publisher pubPathGraph, pubOdomGraph, pubMapGraph;
...
void poseGraphOptimiza()
{
    ...
    while(1)
    {
        if( !cloudQueue.empty() && !odomQueue.empty() )
        {
            ...
            odometryTime = odomQueue.front()->header.stamp.toSec();
            Pose6D currPose = getOdom(odomQueue.front());
            odomQueue.pop();
...
int main(int argc, char** argv)
{
    ...
    ros::Subscriber subLidarCloudFull = nh.subscribe<sensor_msgs::PointCloud2>("/fullCloud", 100, callbackVelodyne);
    ros::Subscriber subLidarOdometry  = nh.subscribe<nav_msgs::Odometry>("/Odometry", 100, callbackOdometry);
