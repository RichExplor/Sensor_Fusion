...
#include "include/featureExtract.hpp"

using namespace std;
using namespace cv;


featureExtract featureExtractFactor;
...
void lidar_process()
{
    while(1)
    {
       if(!lidarQueue.empty())
       {
           ...
           lidarCount++;
           featureExtractFactor.extractFeature(lidarCloudMsg, featureCloud_Edge, featureCloud_Surf);
...
{
    ros::init(argc, argv, "featureExtration");
    ros::NodeHandle nh;
